...
def index():
    ...
    user_names  = load("user_names.json", {})
    users       = get_users()
    ...
    welcome = {**DEFAULT_WELCOME, **load("welcome.json", {})}
...
def add_strike_route():
    ...
    pending = load("pending_strikes.json", [])
    pending.append({
        "user_id": uid,
        "reason": f"{reason} (dashboard — {session.get('username', 'moderator')})",
    })
    save("pending_strikes.json", pending)
...
@edit_required
def save_welcome():
    w = {**DEFAULT_WELCOME, **load("welcome.json", {})}
